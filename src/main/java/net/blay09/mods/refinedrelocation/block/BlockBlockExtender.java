...
public class BlockBlockExtender extends BlockModTile {
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack itemStack) {
        ...
        if (tileEntity instanceof TileBlockExtender) {
            ...
            EnumFacing facing = state.get(DIRECTION);
    ...
    public IBlockState getStateFromMeta(int meta) {
        return getDefaultState().with(DIRECTION, EnumFacing.getFront(meta));
    }

    @Override
    public int getMetaFromState(IBlockState state) { // TileBlockExtender depends on this being an ordinal of EnumFacing
        return state.get(DIRECTION).ordinal();
    }

    @Override
    @SuppressWarnings("deprecation")
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
        return getDefaultState().with(DIRECTION, facing.getOpposite());
    }
