...
public class ContainerMod extends Container implements IContainerNetworked {
    ...
    protected void addPlayerInventory(PlayerEntity player, int offsetY) {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 9; j++) {
                addSlot(new Slot(player.inventory, j + i * 9 + 9, 8 + j * 18, offsetY + i * 18));
            }
        }

        for (int i = 0; i < 9; i++) {
            addSlot(new Slot(player.inventory, i, 8 + i * 18, offsetY + 58));
        }
    }
